...
//todo: add a search filter bar
//todo: fix appended thumbnail magnet links (somtimes they have "undefined" in them)
...
var debugmode = false; // debug mode (setting this to false will disable the console logging)
...
// makes typing stuff in the search box hide non-matching results
//todo: use fuzzy string matching
const searchBox = document.querySelector('#searchinput');
searchBox.updateSearch = function(e){
	const value = this.value;
	torrents = document.querySelectorAll('table > tbody > tr.lista2 a[title]');
	const re = new RegExp(value.replace(/-?[\s\W]+/, '.'), 'i');
		
	for(const a of torrents){
		var match = a.title.match(re);
		console.log('match:', match);
		if(!!match ^ /\-./.test(value))
        {
			console.log('found match "'+value+'" with torrent:', a.title);
			a.closest('.lista2').style.display = '';
        } else {
			a.closest('.lista2').style.display = 'none';
        }
	}
};
searchBox.onkeyup = searchBox.updateSearch;
...
if (matchSite(/\/torrent\//)) {
    ...
    for (const torrent of torrents) {
        ...
        // language=CSS
        addCss(`thumbnail-cell img.preview-image {
...
(function addColumnHeader() {
    ...
    dlAnchor.addEventListener('click', downloadAllTorrents);
    ...
    Mousetrap.bind(["ctrl+s"], (e) => {// saves an html file containing the data
        ...
        var torrentJsons = Array.from(rows).map(row => {
        	...
        	return {
        		...
        		page: a.href,
        		torrentLink: row.querySelector('.torrent-dl').href,
        		magnetLink: encodeURI(row.querySelector('.torrent-ml').href),
        ...
        const summaryHTML = '<html>'+document.head.outerHTML + '<body>'+ getElementsByXPath("//table[@class='lista2t']")[0].outerHTML +'</body></html>';
...
function isPageBlockedKSA() {
    var msgText = document.querySelector('#r4 > td[dir="ltr"].english');
...
// these are how the extra headers look like, these must be removed
/*
<tbody><tr>
...
<td align="center" class="header6 header40"><img src="https://dyncdn2.com/static/20/images/comments.gif" border="0" alt="comments"></td>
